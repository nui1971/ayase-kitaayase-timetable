...
| 2 | ダイヤバッジ | 今日のダイヤ区分を1つ表示（「平日」または「土・休日」）・自動判定のみ・手動切替なし |
...
| 次の列車ラベル | 「次の列車（3・4番線）」と固定表示（番線が不明な便があるため3・4番線で統一） |
...
| 番線バッジ 幅 | 固定幅 `4.5rem`・テキスト中央揃え（「0番線」「3・4番線」で幅を統一） |
...
│   │   ├── TrainRow.test.tsx      # 番線バッジ・時刻・行き先表示テスト
│   │   └── TrackCard.test.tsx     # 次の列車ラベル・終電済み表示テスト
│   ├── App.tsx
│   ├── index.css
│   └── main.tsx
├── docs/
...
| 2026-05-10 | 1.4 | 時刻表データを公式サイト（2026年3月14日改正）に更新・3番線表示を「3・4番線」に統一・番線バッジ固定幅化・テスト追加（48件・6ファイル） |
